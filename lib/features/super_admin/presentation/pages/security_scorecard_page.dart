...
class SecurityScorecardPage extends ConsumerWidget {
            ...
            SliverPadding(
              ...
              sliver: SliverToBoxAdapter(
                child: Column(
                    ...
                    children: [
                      D.header('Security Scorecard',
                          'Platform security health overview', isDark),
                      ...
                      _finding(context, '2FA not enforced on 12 accounts',
                          'Medium', Colors.orange, cardBg),
                      const SizedBox(height: 8),
                      _finding(context, 'API keys expired for 5 orgs', 'Low',
                          Colors.green, cardBg),
                      const SizedBox(height: 8),
                      _finding(context, 'Suspicious login attempts detected',
                          'High', Colors.red, cardBg),
                      const SizedBox(height: 24),
                      D.sectionTitle('Recent Events', Icons.history, isDark),
                      const SizedBox(height: 14),
                      _event(context, 'Password changed', 'admin@org.com',
                          '2h ago', cardBg),
                      _event(context, '2FA enabled', 'user@org.com', '5h ago',
                          cardBg),
                      _event(context, 'API key rotated', 'system', '1d ago',
                          cardBg),
                    ]),
              ),
            ),
  ...
  Widget _gauge(
      BuildContext ctx, String label, int score, Color color, Color cardBg) {
  ...
  Widget _finding(BuildContext ctx, String text, String severity, Color color,
      Color cardBg) {
    final isDark = Theme.of(ctx).brightness == Brightness.dark;
    return Container(
  ...
  Widget _event(
      BuildContext ctx, String action, String user, String time, Color cardBg) {
    final isDark = Theme.of(ctx).brightness == Brightness.dark;
    return Padding(
