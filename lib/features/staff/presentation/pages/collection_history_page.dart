...
class CollectionHistoryPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CollectionHistoryPage> createState() =>
      _CollectionHistoryPageState();
}
...
class _CollectionHistoryPageState extends ConsumerState<CollectionHistoryPage> {
    ...
    return ref.watch(staffProfileProvider).when(
          data: (p) {
            ...
            return ref.watch(collectionHistoryProvider(params)).when(
                  data: (h) =>
                      h.isEmpty ? _empty(theme) : _list(theme, isDark, h),
  ...
  Widget _list(
      ThemeData theme, bool isDark, List<Map<String, dynamic>> history) {
    double total = 0;
    ...
    for (final i in history) {
      final d = (i['collection_date'] as String? ?? '').length >= 10
          ? (i['collection_date'] as String).substring(0, 10)
          : 'Unknown';
      ...
      slivers: [
        ...
        ...entries.asMap().entries.map((g) {
          ...
          final dayTotal = items.fold<double>(0,
              (s, i) => s + ((i['amount_collected'] as num?)?.toDouble() ?? 0));
          return SliverMainAxisGroup(
            slivers: [
              SliverToBoxAdapter(
                  child: _sectionHeader(theme, g.value.key, dayTotal)),
              SliverPadding(
                padding: const EdgeInsets.fromLTRB(20, 12, 20, 20),
                sliver: SliverList(
                  delegate: SliverChildBuilderDelegate(
                    (context, index) =>
                        _card(theme, isDark, items[index], index)
                            .animate()
  ...
  Widget _trendIndicator(bool up) {
    return Container(
      ...
      decoration: BoxDecoration(
          color:
              (up ? AppColors.success : AppColors.error).withValues(alpha: 0.2),
          borderRadius: BorderRadius.circular(8)),
      child: Row(
        children: [
          Icon(up ? Icons.trending_up_rounded : Icons.trending_down_rounded,
              color: up ? AppColors.success : AppColors.error, size: 12),
    ...
    return Center(
      ...
      Container(
          ...
          child: Icon(Icons.receipt_long_outlined,
              size: 60, color: AppColors.primary.withValues(alpha: 0.2))),
      const SizedBox(height: 24),
      Text('No history yet',
          style: TextStyle(
              fontSize: 20,
