...
- **2026-07-14:** Implemented Web/App Lifecycle Reconnection Observer and Stale Marker Age-Out for both `ManagerLiveMapPage` and `LiveAgentsMapCard`. Mixed in `WidgetsBindingObserver` to watch `AppLifecycleState.resumed` (tab sleep / window reactivation recovery) to cleanly unsubscribe, refresh latest coordinates snapshot, and resubscribe. Integrated a periodic 10-second `Timer` ticker trigger (calling `setState`) to dynamically age-out inactive and lagging agents (older than 5 minutes) to `offline` status (grey markers and decremented active stats tally). `flutter analyze` clean.
