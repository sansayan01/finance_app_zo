  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return trendAsync.when(
      data: (trend) {
        ...
        final yesterdayAmount = amounts.length >= 2 ? amounts[amounts.length - 2] : 0.0;
        final todayAmount = amounts.last;
        final pctChange = yesterdayAmount > 0 ? ((todayAmount - yesterdayAmount) / yesterdayAmount * 100) : 0.0;
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text('Performance Pulse', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w800)),
                  if (pctChange != 0)
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                      decoration: BoxDecoration(
                        color: (pctChange >= 0 ? Colors.greenAccent : Colors.redAccent).withValues(alpha: 0.15),
                        borderRadius: BorderRadius.circular(6),
                      ),
                      child: Text(
                        '${pctChange >= 0 ? '+' : ''}${pctChange.toStringAsFixed(1)}% Today',
                        style: TextStyle(
                          color: pctChange >= 0 ? Colors.greenAccent : Colors.redAccent,
                          fontSize: 10,
              ...
              SizedBox(
                ...
                child: Row(
                  ...
                  children: List.generate(trend.length, (index) {
                    ...
                    final barHeight = maxVal > 0 ? (amount.toDouble() / maxVal) * 50 : 0.0;
