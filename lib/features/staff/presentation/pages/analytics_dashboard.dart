...
class _AnalyticsDashboardState extends ConsumerState<AnalyticsDashboard> {
  ...
  Widget _buildSummaryCards(ThemeData theme, bool isDark) {
    ...
    return statsAsync.when(
      data: (stats) {
        ...
        final yesterdayTotal = trendAsync.valueOrNull != null &&
                trendAsync.valueOrNull!.length >= 2
            ? (trendAsync.valueOrNull![trendAsync.valueOrNull!.length - 2]
                        ['amount'] as num?)
                    ?.toDouble() ??
                0.0
            : 0.0;
        final chg = yesterdayTotal > 0
        ...
        return Row(
          children: [
            Expanded(
                child: _buildSummaryCard(theme, 'Collected', '₹${_fmt(total)}',
                    chg, AppColors.success, Icons.trending_up_rounded, isDark)),
            const SizedBox(width: 10),
            Expanded(
                child: _buildSummaryCard(theme, 'Count', '$count', 0.0,
                    AppColors.info, Icons.receipt_long_rounded, isDark)),
            const SizedBox(width: 10),
            Expanded(
                child: _buildSummaryCard(theme, 'Average', '₹${_fmt(avg)}', 0.0,
                    AppColors.accent, Icons.payments_rounded, isDark)),
          ],
        );
      },
  ...
  Widget _buildSummaryCard(ThemeData theme, String title, String value,
      double change, Color color, IconData icon, bool isDark) {
          ...
          Row(
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                    _selectedPeriod == 'week' ? 'This Week' : _selectedPeriod,
                    style: TextStyle(
            ...
            data: (trend) {
              ...
              return Column(
                children: [
                  ...
                  Text('Total: ₹${_fmt(amounts.fold(0.0, (a, b) => a + b))}',
                      style: TextStyle(
                          fontSize: 11,
                          color: theme.colorScheme.onSurface
    ...
    final metrics = [
      {
        ...
        'value': '${(targetProgress * 100).toStringAsFixed(0)}%',
        'progress': targetProgress.clamp(0.0, 1.0)
      },
      {
        'label': 'Collection Streak',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          ...metrics.map((m) => _buildMetricRow(theme, m['label'] as String,
              m['value'] as String, m['progress'] as double, isDark)),
        ],
      ),
    );
  }
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          height: 26,
                          decoration: BoxDecoration(
                              color: rankColors[i].withValues(alpha: 0.2),
                        ...
                        Expanded(
                          child: Column(
                              ...
                              children: [
                                ...
                                if (outstanding > 0)
                                  Text(
                                      '₹${outstanding.toStringAsFixed(0)} outstanding',
                                      style: TextStyle(
                                          fontSize: 10,
                                          color: theme.colorScheme.onSurface
                                              .withValues(alpha: 0.4))),
