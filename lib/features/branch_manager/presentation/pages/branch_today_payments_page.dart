                            ...
                            : () async {
                                ...
                                try {
                                  ...
                                  if (user?.branchId != null) {
                                    ref.invalidate(
                                        branchTodayPaymentsProvider(
                                            user!.branchId!));
                                    ref.invalidate(
                                        branchCollectionStatsProvider(
                                            user.branchId!));
                                    ref.invalidate(
                                        branchLoansProvider(
                                            user.branchId!));
                                    ref.invalidate(
                                        branchSavingsProvider(
                                            user.branchId!));
                                    ref.invalidate(
                                        branchStatsProvider(
                                            user.branchId!));
  ...
  Future<void> _recordCollection({
    ...
    if (payment.type == PaymentType.savings) {
      await client.from('savings_collections').insert({
        ...
        'is_partial':
            amount < (payment.amountExpected * installmentCount),
      ...
      switch (collectionType) {
        ...
        case 'monthly':
          final targetDay =
              plan?['collection_day_of_month'] ?? now.day;
          ...
          final daysInMonth =
              DateTime(targetYear, adjustedMonth + 1, 0).day;
          nextDue = DateTime(
              targetYear,
              adjustedMonth,
      ...
      if (payment.loanId != null) {
        // EMI marking handled by DB trigger 'update_schedule_on_collection'
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      if (payment.isCollected &&
                        ...
                        const SizedBox(height: 2),
                        Text(timeFormat.format(payment.collectedAt!),
                            style: TextStyle(
                                fontSize: 10,
                                color: isDark
                                    ? AppColors.textTertiaryDark
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color:
                isSelected ? AppColors.success : Colors.grey.shade300,
