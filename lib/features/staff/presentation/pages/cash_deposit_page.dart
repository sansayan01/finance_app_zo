...
class _CashDepositPageState extends ConsumerState<CashDepositPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Deposit Cash', style: TextStyle(fontWeight: FontWeight.w800)),
        centerTitle: false,
      ),
      body: user == null
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text('Cash in Hand', style: TextStyle(color: Colors.white.withValues(alpha: 0.7), fontWeight: FontWeight.w500)),
                Text(NumberFormat.currency(locale: 'en_IN', symbol: '₹').format(wallet.cashInHand),
                    style: const TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.w900)),
              ],
            ),
          ),
          TextButton(
            onPressed: () => _amountController.text = wallet.cashInHand.toStringAsFixed(0),
            ...
            child: const Text('All', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
          ),
        ],
      ),
    );
  ...
  Widget _buildAmountSection(ThemeData theme, bool isDark) {
    return _section(theme, isDark, 'Amount', Icons.payments_rounded,
      child: TextFormField(
        ...
        decoration: InputDecoration(
          prefixText: '₹ ',
          prefixStyle: theme.textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.w900, color: AppColors.primary),
          hintText: '0',
  ...
  Widget _buildMethodSection(ThemeData theme, bool isDark) {
    final methods = [
      {'id': 'branch', 'icon': Icons.account_balance_rounded, 'label': 'Branch Deposit'},
      {'id': 'bank', 'icon': Icons.account_balance_wallet_rounded, 'label': 'Bank Transfer'},
      {'id': 'upi', 'icon': Icons.qr_code_rounded, 'label': 'UPI'},
    ];

    return _section(theme, isDark, 'Deposit Method', Icons.swap_horiz_rounded,
      child: Row(
        children: methods.map((m) {
          final isSelected = _depositMethod == m['id'];
          return Expanded(
            child: Padding(
              padding: EdgeInsets.only(right: m['id'] != methods.last['id'] ? 8 : 0),
              child: GestureDetector(
                onTap: () { HapticFeedback.selectionClick(); setState(() => _depositMethod = m['id'] as String); },
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 200),
                  padding: const EdgeInsets.symmetric(vertical: 16),
                  decoration: BoxDecoration(
                    color: isSelected ? AppColors.primary.withValues(alpha: 0.12) : (isDark ? Colors.white.withValues(alpha: 0.05) : theme.colorScheme.surface),
                    borderRadius: BorderRadius.circular(16),
                    border: Border.all(color: isSelected ? AppColors.primary : Colors.transparent, width: 1.5),
                  ),
                  child: Column(
                    children: [
                      Icon(m['icon'] as IconData, color: isSelected ? AppColors.primary : theme.colorScheme.onSurface.withValues(alpha: 0.4), size: 22),
                      const SizedBox(height: 6),
                      Text(m['label'] as String, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: isSelected ? AppColors.primary : null)),
                    ],
                  ),
                ),
              ),
  ...
  Widget _buildReferenceSection(ThemeData theme, bool isDark) {
    return _section(theme, isDark, 'Reference', Icons.tag_rounded,
      child: TextField(
  ...
  Widget _buildNotesSection(ThemeData theme, bool isDark) {
    return _section(theme, isDark, 'Notes', Icons.notes_rounded,
      child: TextField(
      ...
      child: ElevatedButton(
        ...
        child: _isLoading
            ...
            : const Row(
                ...
                children: [Icon(Icons.upload_rounded, size: 22), SizedBox(width: 10), Text('Confirm Deposit', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700))],
              ),
      ),
    );
  }

  Widget _section(ThemeData theme, bool isDark, String title, IconData icon, {required Widget child}) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
