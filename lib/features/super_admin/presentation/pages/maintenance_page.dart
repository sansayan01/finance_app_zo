...
class _MaintenancePageState extends ConsumerState<MaintenancePage> {
                ...
                child: Column(
                    ...
                    children: [
                      Row(
                          ...
                          children: [
                            D.header('Maintenance', 'Schedule downtime windows',
                                isDark),
  ...
  Widget _card(dynamic w, bool isDark, Color cardBg) {
    ...
    final ongoing =
        now.isAfter(w.scheduledStart) && now.isBefore(w.scheduledEnd);
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
              ...
              decoration: BoxDecoration(
                  color: (w.isActive ? Colors.orange : D.mutedColor(isDark))
          ...
          Switch(
              value: w.isActive,
              onChanged: (v) => _toggle(w.id, v),
              activeTrackColor: Colors.orange),
        ]),
        if (w.description != null) ...[
          const SizedBox(height: 10),
          Text(w.description!, style: D.valueStyle(isDark))
        ],
        const SizedBox(height: 14),
        Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(
              color: D.fill(context), borderRadius: BorderRadius.circular(8)),
          child: Row(children: [
            ...
            Expanded(
                child: Column(
                    ...
                    children: [
                  Text(
                      'Start: ${DateFormat('MMM d, y • h:mm a').format(w.scheduledStart)}',
                      style: D.subtitleStyle(isDark)),
                  Text(
                      'End: ${DateFormat('MMM d, y • h:mm a').format(w.scheduledEnd)}',
                      style: D.subtitleStyle(isDark)),
                ])),
            Text('${w.duration.inHours}h', style: D.titleStyle(isDark)),
          ]),
        ),
        if (w.affectedServices.isNotEmpty) ...[
          const SizedBox(height: 10),
          Wrap(
              spacing: 6,
              runSpacing: 4,
              children: w.affectedServices
                ...
                content: SingleChildScrollView(
                  ...
                  DropdownButtonFormField<int>(
                      ...
                      items: [1, 2, 3, 4, 6, 8, 12, 24]
                          ...
                          .toList(),
                      onChanged: (v) => setS(() => durationHours = v ?? 2)),
                ])),
                actions: [
                  TextButton(
                      ...
                      onPressed: () async {
                        ...
                        await ref
                            ...
                            .createMaintenanceWindow(
                                title: titleC.text,
                                description:
                                    descC.text.isEmpty ? null : descC.text,
