...
class AdminOrgDetailPage extends ConsumerWidget {
  ...
  Widget _buildContent(BuildContext context, Map<String, dynamic> org, bool isDark) {
  ...
  Widget _buildHeaderCard(Map<String, dynamic> org, String status, Color statusColor, bool isDark) {
    final name = org['name'] as String? ?? '';
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(status[0].toUpperCase() + status.substring(1), style: TextStyle(
                  fontSize: 12, fontWeight: FontWeight.w700, color: statusColor, letterSpacing: 0.3)),
              ],
            ),
          ),
          const SizedBox(height: 12),
          Text('$slug  •  Created $dateStr', style: TextStyle(
        ...
        children: [
          Row(
            children: [
              ...
              Text('Organization Info', style: TextStyle(
                fontSize: 16, fontWeight: FontWeight.w700,
                color: isDark ? Colors.white : const Color(0xFF0F172A))),
            ],
          ),
          const SizedBox(height: 16),
          _infoTile(Icons.alternate_email_rounded, 'Slug', org['slug'] as String? ?? '', isDark),
          _infoTile(Icons.calendar_today_rounded, 'Created', _formatDate(org['created_at'] as String?), isDark),
          _infoTile(Icons.update_rounded, 'Updated', _formatDate(org['updated_at'] as String?), isDark),
        ],
      ),
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text('Plan Limits', style: TextStyle(
                fontSize: 16, fontWeight: FontWeight.w700,
                color: isDark ? Colors.white : const Color(0xFF0F172A))),
            ],
          ),
          ...
          Row(
            children: [
              Expanded(child: _limitChip(Icons.business_rounded, 'Branches', '${org['max_branches'] ?? 5}', AppColors.primary, isDark)),
              const SizedBox(width: 12),
              Expanded(child: _limitChip(Icons.people_rounded, 'Staff', '${org['max_staff'] ?? 20}', AppColors.success, isDark)),
              const SizedBox(width: 12),
              Expanded(child: _limitChip(Icons.person_rounded, 'Members', '${org['max_members'] ?? 500}', AppColors.warning, isDark)),
            ],
          ),
        ],
      ),
      ...
      child: Column(
        children: [
          ...
          Text(value, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800,
            color: isDark ? Colors.white : const Color(0xFF0F172A))),
          Text(label, style: TextStyle(fontSize: 11, color: isDark ? Colors.grey.shade500 : Colors.grey.shade600)),
        ],
      ),
    );
  }
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 80, child: Text(label, style: TextStyle(
            fontSize: 14, color: isDark ? Colors.grey.shade500 : Colors.grey.shade600))),
  ...
  Future<void> _suspendOrg(BuildContext context, String id) async {
    try {
      ...
      await client.from('organizations').update({'status': 'suspended'}).eq('id', id);
      if (context.mounted) {
