...
class _OverdueListPageState extends ConsumerState<OverdueListPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: false,
        actions: [
          _chip(theme, Icons.filter_alt_rounded,
              _filter == 'all' ? 'All' : _filter, () => _sheet(theme, true)),
  ...
  Widget _dangerMeter(
      ThemeData theme, List<Map<String, dynamic>> items, bool isDark) {
    double total = 0;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
              color: AppColors.error.withValues(alpha: isDark ? 0.2 : 0.08),
              blurRadius: 30,
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    _meterSubItem(
                        theme,
                        '${((critical / items.length.clamp(1, 1000)) * 100).toStringAsFixed(0)}%',
                        'RISK %',
                        isDark),
                  ],
                ),
                ...
                Text('Last updated 2 mins ago • Trends are up by 12%',
                    style: TextStyle(
                        fontSize: 9,
                        color:
  ...
  Widget _card(Map<String, dynamic> c, ThemeData theme, bool isDark, int i) {
    ...
    final color = days > 30
        ...
        : (days > 15
            ? AppColors.warning
            : (days > 7 ? AppColors.orange : AppColors.info));
            ...
            child: Column(
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                        child: _btn(theme, Icons.phone_rounded, 'CALL',
                            AppColors.primary, true, () => _call(phone))),
                    const SizedBox(width: 8),
                    Expanded(
                        child: _btn(theme, Icons.map_rounded, 'VISIT',
                            AppColors.info, true, () => _visit())),
                    const SizedBox(width: 8),
                    Expanded(
                      flex: 2,
                      child: _btn(theme, Icons.payments_rounded, 'COLLECT',
                          color, false, () {
                        HapticFeedback.mediumImpact();
                        context.push('/staff/collection/${c['loan_id']}',
                            extra: c);
                      }),
                    ),
                  ],
  ...
  Widget _btn(ThemeData theme, IconData icon, String label, Color color,
      bool outlined, VoidCallback onTap) {
    return SizedBox(
      height: 46,
      child: outlined
          ? OutlinedButton.icon(
    ...
    return Center(
      ...
      Container(
          ...
          child: Icon(Icons.check_circle_outline_rounded,
              size: 64, color: AppColors.success.withValues(alpha: 0.3))),
      const SizedBox(height: 24),
      Text('All Clear!',
          style: TextStyle(
              fontSize: 22,
  ...
  List<Map<String, dynamic>> _sortBy(List<Map<String, dynamic>> items) {
    ...
    if (_sort == 'days') {
      list.sort((a, b) => (b['days_overdue'] as int? ?? 0)
          .compareTo(a['days_overdue'] as int? ?? 0));
    } else if (_sort == 'amount') {
      list.sort(
          (a, b) => (b['emi'] as num? ?? 0).compareTo(a['emi'] as num? ?? 0));
    } else if (_sort == 'name') {
      list.sort((a, b) => (a['member_name'] as String? ?? '')
          .compareTo(b['member_name'] as String? ?? ''));
    }
    return list;
  }
      ...
      builder: (ctx) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Align(
              ...
              child: Text(isFilter ? 'Filter' : 'Sort',
          ...
          Wrap(
            ...
            children: isFilter
                ...
                : [
                    _sortPill(
                        'days', 'Most Overdue', Icons.calendar_today_rounded),
                    _sortPill(
                        'amount', 'Highest Amount', Icons.payments_rounded),
                    _sortPill(
                        'name', 'Alphabetical', Icons.sort_by_alpha_rounded),
                  ],
          ),
        ]),
      ),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: SnackBarAction(
            label: 'GO', onPressed: () => context.push('/staff/router')),
      ),
    );
  }

  String _fmt(double n) => n >= 100000
