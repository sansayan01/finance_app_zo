...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ── 2. Read platform chatbot config (service role) ────────────
    ...
    const modelId = (config.model_id as string) || "meta/llama-3.1-70b-instruct";
    const chatbotEnabled = config.enabled !== false; // default true
    ...
    // ── 4. Get user profile for role-scoped context ───────────────
    ...
    // ── 5. Build role-scoped business context ─────────────────────
    const orgName_ = orgName || "MicroFlow Pro";
    let systemPrompt =
      `You are the ${orgName_} Assistant, a concise multilingual financial expert. ` +
    ...
    try {
      if (profile.role === "superAdmin") {
        ...
        // Staff sees their own collections
      ...
      // Context build failure is non-fatal — continue without context
    ...
    // ── 6. Call NVIDIA NIM with streaming ─────────────────────────
