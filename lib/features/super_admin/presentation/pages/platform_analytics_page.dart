...
class PlatformAnalyticsPage extends ConsumerWidget {
          ...
          slivers: [
            ...
            metrics.when(
              data: (m) => SliverPadding(
                ...
                sliver: SliverToBoxAdapter(
                    child: _metricGrid(context, m, fmt, isDark, cardBg)),
              ),
            ...
            revenue.when(
              data: (r) => SliverPadding(
                padding: const EdgeInsets.fromLTRB(24, 0, 24, 16),
                sliver: SliverToBoxAdapter(
                    child: _revCard(context, r, fmt, isDark, cardBg)),
              ),
              loading: () => const SliverToBoxAdapter(child: SizedBox.shrink()),
              error: (_, __) =>
                  const SliverToBoxAdapter(child: SizedBox.shrink()),
            ...
            apiStats.when(
              data: (a) => SliverPadding(
                ...
                sliver: SliverToBoxAdapter(
                    child: _apiCard(context, a, isDark, cardBg)),
              ),
              loading: () => const SliverToBoxAdapter(child: SizedBox.shrink()),
              error: (_, __) =>
                  const SliverToBoxAdapter(child: SizedBox.shrink()),
  ...
  Widget _metricGrid(BuildContext context, dynamic m, NumberFormat fmt,
  ...
  Widget _revCard(BuildContext context, Map<String, dynamic> r,
      NumberFormat fmt, bool isDark, Color cardBg) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text('Revenue', style: D.titleStyle(isDark))
        ]),
        const SizedBox(height: 20),
        Row(
            children: [
          _rItem(context, 'Total', fmt.format(r['total_revenue'] ?? 0),
              Colors.green),
          _rItem(context, 'Avg Monthly',
              fmt.format(r['avg_monthly_revenue'] ?? 0), Colors.blue),
          _rItem(context, 'Transactions', '${r['transaction_count'] ?? 0}',
              Colors.orange),
        ]
                .expand((w) => [
                      w,
                      Container(width: 1, height: 30, color: D.border(context))
                    ])
                .toList()
              ..removeLast()),
      ]),
  ...
  Widget _rItem(BuildContext context, String label, String value, Color color) {
    ...
    return Expanded(
      ...
      Text(label, style: D.subtitleStyle(isDark))
    ]));
  }

  Widget _apiCard(
      BuildContext context, Map<String, dynamic> a, bool isDark, Color cardBg) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
          color: cardBg,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Icon(Icons.api, size: 18, color: Colors.indigo),
          const SizedBox(width: 8),
          Text('API Usage (7 days)', style: D.titleStyle(isDark))
        ]),
        const SizedBox(height: 20),
        Row(children: [
          Expanded(
              child: Column(children: [
            Text('${a['total_requests'] ?? 0}', style: D.h2(isDark)),
            Text('Requests', style: D.subtitleStyle(isDark))
          ])),
          Expanded(
              child: Column(children: [
            Text('${a['unique_endpoints'] ?? 0}', style: D.h2(isDark)),
            Text('Endpoints', style: D.subtitleStyle(isDark))
          ])),
          Expanded(
              child: Column(children: [
            Text('${(a['avg_response_time'] ?? 0).toStringAsFixed(0)}ms',
                style: D.h2(isDark)),
            Text('Avg Response', style: D.subtitleStyle(isDark))
          ])),
        ]),
      ]),
    );
