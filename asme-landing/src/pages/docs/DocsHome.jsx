...
import {
  ...
  Play, FileText, BookOpen, ArrowRight, Wallet, BarChart3
} from "lucide-react";
...
export default function DocsHome() {
  ...
  const guideCards = [
    ...
    },
    {
      ...
      icon: BarChart3,
      path: "/docs/tracking-repayments",
      iconWrap: "icon-wrap-amber",
      lessonsCount: 4,
    },
    {
      title: "SMS Reminders",
      desc: "Set up automatic repayment reminders for your borrowers.",
      icon: UserCircle,
      path: "/docs/sms-reminders",
      iconWrap: "icon-wrap-violet",
      lessonsCount: 4,
    },
    {
      title: "Portfolio Insights",
      desc: "Understand your portfolio — total lent, outstanding, interest earned, and repayment rate.",
      icon: Smartphone,
      path: "/docs/portfolio-insights",
      iconWrap: "icon-wrap-pink",
      lessonsCount: 4,
    },
  ];
    ...
    <div className="space-y-16">
      ...
      <div className="relative">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500/12 to-violet-500/5 border border-indigo-500/10 flex items-center justify-center">
          ...
          <div>
            ...
            <p className="text-tertiary text-xs mt-0.5">{t.home.subtitle}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 mt-6">
          <div className="flex items-center gap-2 px-3.5 py-2 rounded-full bg-white/[0.03] border border-white/[0.05]">
            ...
            <span className="text-xs text-secondary"><strong className="text-primary/80">25</strong> {t.common.lessons}</span>
          </div>
          <div className="flex items-center gap-2 px-3.5 py-2 rounded-full bg-white/[0.03] border border-white/[0.05]">
            <span className="w-1.5 h-1.5 rounded-full bg-cyan-400" />
            <span className="text-xs text-secondary"><strong className="text-primary/80">6</strong> {t.common.portals}</span>
          </div>
          <div className="flex items-center gap-2 px-3.5 py-2 rounded-full bg-white/[0.03] border border-white/[0.05]">
            <span className="w-1.5 h-1.5 rounded-full bg-red-400" />
            <span className="text-xs text-secondary"><strong className="text-primary/80">Video</strong> tutorials</span>
          </div>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="text-[10px] font-semibold text-tertiary uppercase tracking-[0.2em]">{t.common.selectPortal}</span>
          <div className="h-px flex-1 bg-gradient-to-l from-indigo-500/15 to-transparent" />
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {guideCards.map((card) => (
            <Link
              key={card.path}
              to={card.path}
              className="group relative rounded-2xl border border-white/[0.04] bg-white/[0.01] hover:bg-white/[0.02] transition-all duration-500 overflow-hidden premium-glass-card"
            >
              {/* Hover glow */}
              ...
              <div className="relative p-5 sm:p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-primary font-semibold text-base mb-1.5 group-hover:text-indigo-200 transition-colors">
                      {card.title}
                    </h3>
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/[0.03]">
                  ...
                  <span className="text-xs text-tertiary group-hover:text-indigo-300 transition-colors flex items-center gap-1">
      ...
      <div>
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="text-sm font-medium text-secondary">{t.common.popularGuides}</span>
        </div>
        <div className="grid sm:grid-cols-2 gap-2.5">
          {t.home.topics.map((topic, i) => (
            <Link
              key={i}
              to={topic.path}
              className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.01] border border-white/[0.04] hover:border-indigo-500/12 hover:bg-indigo-500/[0.02] text-secondary hover:text-primary text-sm transition-all duration-200 group"
            >
      ...
      {/* YouTube CTA */}
      <div className="relative rounded-2xl overflow-hidden border border-red-500/10 bg-gradient-to-br from-red-500/[0.03] to-transparent premium-glass-card">
        ...
        <div className="relative p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-primary font-semibold text-lg mb-1">{t.common.youtubeCtaTitle}</h3>
