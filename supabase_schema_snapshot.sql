...
BEGIN
  ...
  -- profile_id is NULL — try to resolve via auth.uid() -> profiles.user_id
...
COMMENT ON COLUMN "public"."upi_payment_requests"."installment_date" IS 'The due date this payment is covering — applies to both loan EMIs and savings installments.';
